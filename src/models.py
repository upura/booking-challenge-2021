...
class BookingNNMtl(nn.Module):
    def __init__(
        ...
        self.ffn_h = nn.Sequential(
            ...
            nn.Dropout(dropout),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_size, n_hotel_id),
        )
        self.n_city_id = n_city_id
        self.n_hotel_id = n_hotel_id
        self.n_booker_country = n_booker_country
        self.emb_dim = emb_dim
        self.rnn_dim = rnn_dim
        self.num_layers = num_layers
    ...
    def forward(
        ...
        out_s = torch.cat([cate_emb, cont_emb], dim=2)
        out_h = torch.cat([cate_emb, cont_emb], dim=2)

        out_s, _ = self.lstm(out_s)
        out_h, _ = self.lstm(out_h)

        out_s = out_s[:, -1, :]  # extrast last value of sequence
        out_h = out_h[:, -1, :]  # extrast last value of sequence

        out_s = self.ffn(out_s)
        out_h = self.ffn_h(out_h)

        return (out_s, out_h)


class BookingNNAug(nn.Module):
    def __init__(
        self,
        n_city_id,
        n_hotel_id,

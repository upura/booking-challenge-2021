...
if __name__ == '__main__':
    ...
    train_test['hotel_country'] = hotel_le.fit_transform(train_test['hotel_country'].fillna('-1'))
    ...
    train_hotels = train[train['city_id'] != train['city_id'].shift(1)].groupby('utrip_id')['hotel_country'].apply(lambda x: x.values).reset_index()
    test_hotels = test[test['city_id'] != test['city_id'].shift(1)].query('city_id!=0').groupby('utrip_id')['hotel_country'].apply(lambda x: x.values).reset_index()
    ...
    X_train['hotel_country'] = train_hotels['hotel_country']
    X_test['hotel_country'] = test_hotels['hotel_country']
    ...
    for fold_id, (tr_idx, va_idx) in enumerate(cv.split(X_train,
        ...
        if fold_id in (0,):
            ...
            model = BookingNN(vocab_size=len(target_le.classes_),
                              vocab_size_h=len(hotel_le.classes_))
            ...
            for loop_i, prediction in enumerate(
                ...
                correct = (
                    y_val.values[loop_i]
                    in np.argsort(prediction[0].cpu().numpy()[-1, :])[-4:]
                )
                score += int(correct)
            score /= len(y_val)
